...
public class BattleManager : MonoBehaviour
{
    ...
    void GeneratePlayer()
    {
        ...
        // player = playerObj;
        player = playerObj;
        pm = playerObj.gameObject.GetComponent<PlayerManager>();
        pm.GeneratePlayerDeck();
        pm.DrawCards();
    ...
    {
        print("use card with " + card.name + " on " + target.name);
        if (player.action == 0)
        {
        ...
        if (card.type == Card.Type.ATTACK)
        {
            ...
            {
                print("Area dmg");
                enemies[0].GetComponent<HealthManager>().TakeDamage(card.stat);
                enemies[1].GetComponent<HealthManager>().TakeDamage(card.stat);
            }
            else
            {
                print("ATTACK with " + card.stat);
                target.GetComponent<HealthManager>().TakeDamage(card.stat);
            }
        }
        if (card.type == Card.Type.ARMOR)
        {
            player.GetComponent<HealthManager>().ArmorUp(card.stat);
            print("ARMOR UP " + card.stat + " On ");
        }
        player.action -= 1;
        player.GetComponentInChildren<ActionPoints>().RefreshCount();
    }
